  ...
  <div class="zms-showTicketManagement" :class="nmNightClass">
    <div class="zms-query-filter">
      ...
      <v-dialog v-model="submitStat" persistent width="300">
        <v-card>
          <v-card-title>
            {{ $t("showTicketManagement.Submitting") }}
          </v-card-title>
          <v-divider />
          <br />
          <v-card-text>
            {{ $t("showTicketManagement.PleaseWait") }} <br />
            <br />
            <v-progress-linear
              indeterminate
              striped
      ...
      <v-dialog v-model="noNoteWarning" persistent width="500">
        <v-card color="" :ripple="{ class: null }">
          ...
          >
            ...
            >
              {{ $t("showTicketManagement.NoNoteTitle") }}
            </span>
          </v-card-title>
          <v-divider />
          <br />
          <v-card-text>
            <span class="zms-poptip-body" :class="txNightClass">
              {{ $t("showTicketManagement.NoNoteInfo") }}
            </span>
            <br />
            <br />
          </v-card-text>
        ...
        <v-container>
          <v-row>
            <v-col cols="12" sm="6" md="3">
              <v-text-field
                v-model="submit_showId"
                :label="$t('showTicketManagement.showId')"
                :placeholder="
                  $t('common.pleaseInput') + $t('showTicketManagement.showId')
                "
            ...
            <v-col cols="12" sm="6" md="3">
              <v-text-field
                ...
                :label="$t('showTicketManagement.price')"
                :placeholder="
                  $t('common.pleaseInput') + $t('showTicketManagement.price')
                "
            ...
            <v-col cols="12" sm="6" md="3">
              <v-text-field
                v-model="submit_ticketType"
                :label="$t('showTicketManagement.ticketType')"
                :placeholder="
                  $t('common.pleaseInput') +
                  $t('showTicketManagement.ticketType')
                "
            ...
            <v-col cols="12" sm="6" md="3">
              <v-text-field
                v-model="submit_amount"
                :label="$t('showTicketManagement.amount')"
                :placeholder="
                  $t('common.pleaseInput') + $t('showTicketManagement.amount')
                "
